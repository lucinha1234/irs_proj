...
def load_csv(path):
    return pd.read_csv(path).fillna("").sort_values(by=list(pd.read_csv(path).columns)).reset_index(drop=True)
...
def load_xml(xml_path):
    ...
    df = pd.DataFrame(rows).fillna("").sort_values(by=sorted(df.columns)).reset_index(drop=True)
    return df
...
def test_wide_to_xml_conversion(output_long_file):
    ...
    result_df = load_csv(output_long_file)
    print("result_df:",result_df)
    expected_df = load_csv(expected_xml)
